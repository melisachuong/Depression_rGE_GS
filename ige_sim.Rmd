...
```{r}
prs_scale <- 10
maternal_prs = scale(rnorm(n, mean=maternal_genetic, sd=sqrt(prs_scale*V_A)))[,1]
paternal_prs = scale(rnorm(n, mean=paternal_genetic, sd=sqrt(prs_scale*V_A)))[,1]
```
...
```{r}
offspring_prs = scale((maternal_prs + paternal_prs)/2 + rnorm(n, mean=offspring_segregation, sd=sqrt(prs_scale*V_A))/4)[,1]
```
