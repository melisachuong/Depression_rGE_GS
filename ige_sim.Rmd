...
V_nTagged = h2 - V_Tagged
V_A = V_Tagged + V_nTagged
V_E = (1-h2)*V_P
n = 10000
...
maternal_genetic_forPRS = rnorm(n, mean=0, sqrt(V_Tagged))
paternal_genetic_forPRS = rnorm(n, mean=0, sqrt(V_Tagged))
...
set.seed(14) 
prs_scale <- 10
maternal_prs = scale(rnorm(n, mean=maternal_genetic_forPRS, sd=sqrt(prs_scale*V_Tagged)))[,1]
paternal_prs = scale(rnorm(n, mean=paternal_genetic_forPRS, sd=sqrt(prs_scale*V_Tagged)))[,1]
```
...
```{r}
offspring_prs = scale((maternal_prs + paternal_prs)/2 + rnorm(n, mean=offspring_segregation, sd=sqrt(prs_scale*V_Tagged))/4)[,1]
```
